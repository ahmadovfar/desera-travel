...
export function renderBlog() {
  ...
  return '<div style="padding:140px 0 80px;background:linear-gradient(135deg,var(--dk),#1f2937)"><div class="con" style="text-align:center"><div class="stag" style="color:var(--c1)">Blog</div><h1 style="font-family:var(--fd);font-size:clamp(36px,5vw,52px);color:#fff">' + (E ? 'Полезное для партнёров' : 'Insights for partners') + '</h1></div></div><section class="sec"><div class="con"><div class="sgrid">' + posts.map(function (p) { return '<div class="sv anim"><div class="sv-img"><img loading="lazy" src="' + p.img + '"></div><div class="sv-b"><div style="display:flex;gap:8px;margin-bottom:12px"><span style="font-size:11px;font-weight:600;background:var(--c1l);color:var(--c1);padding:4px 10px;border-radius:100px">' + p.cat + '</span><span style="font-size:11px;color:var(--g500)">' + p.d + '</span></div><div class="sv-t">' + p.t + '</div><p class="sv-p">' + p.ex + '</p></div></div>'; }).join('') + '</div></div></section>';
}
